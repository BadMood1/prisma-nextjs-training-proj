...
export default async function JobsPage({
    ...
    // Параметры попадают в URL автоматически при отправке формы
    // из-за поля name
    console.log(await searchParams);
    const { q, type, location } = await searchParams; // берёт из url'а значения после q, type, location

    // Приводим значение поиска к строке или undefined, чтобы использовать его безопасно в запросе
        ...
        where: {
            AND: [
                query
                    ? {
                          OR: [
                              { title: { contains: query, mode: "insensitive" } }, // Проверяем содержит ли слово из поиска
